...
const ChatWidget = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`AI request failed ${response.status}: ${assistantText}`);
      }
      ...
      const message = error instanceof Error ? error.message : 'Unable to send message. Please try again later.';
      setMessages((current) => [
        ...current,
        {
          id: `a-${Date.now()}`,
          ...
          <div className="max-h-96 space-y-3 overflow-y-auto px-4 py-4 text-sm text-slate-100">
            ...
            <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-center">
              <button
                type="submit"
                disabled={isLoading}
                className="w-full rounded-full bg-amber-400 px-4 py-3 text-sm font-semibold text-slate-950 shadow-lg shadow-amber-500/20 transition hover:bg-amber-300 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
              >
